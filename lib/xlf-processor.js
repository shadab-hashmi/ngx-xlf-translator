const xlfTranslator = require("./ngx-xlf-translator");
 ...
 * @param callback
 */
XlfProcessor.prototype.translateAndProcessFilesWithGoogleApi = function (
  toLanguages,
  callback
) {
  async.waterfall(
    [
      ...
      (xlfSourceFile, callback) => {
        this.translateLanguagesAndCreateResources(
          toLanguages,
          xlfSourceFile,
          (err) => {
            callback(err);
 ...
 * @param done
 */
XlfProcessor.prototype.translateLanguagesAndCreateResources = function (
  languages,
  xlfSourceFile,
  done
) {
  ...
  translatorConfig.fromLanguage = translatorConfig.fromLanguage
    ? translatorConfig.fromLanguage
  ...
  async.eachLimit(
    languages,
    1,
    (languageToTranslate, next) => {
      async.waterfall(
        [
          (callback) => {
            xlfTranslator.translateBody(
              bodyArray,
              translatorConfig.fromLanguage,
              languageToTranslate,
              (err, newBody) => {
          ...
          (newBody, callback) => {
            const path = [
              ...
              constants.FILE_OUTPUT_DIRECTORY,
            ].join("/");
            xlfFileProcessor.createCsvOutputFilesFromBody(
              path,
              newBody,
              languageToTranslate,
              (err) => {
 ...
 * @param callback
 */
XlfProcessor.prototype.checkAndUpdateMessagesIfAvailableFromSourceForTranslations =
  function (done) {
    let sourceMessages;
    ...
    async.waterfall(
      [
        ...
        (callback) => {
          ...
          xlfFileProcessor.getXlFileForLocale(
            localeFirstFile,
            (err, firstXlfFile) => {
        ...
        (callback) => {
          ...
          if (!sourceMessages || !firstFileMessages) {
            const noFilesError = new Error(
              "Message files or the source file seems to be empty"
            );
          ...
          if (sourceMessages.length === firstFileMessages.length) {
            console.log(
              logSymbols.success,
              chalk.gray("No missing files found in source file")
            );
            return done();
          }

          const firstFileSourceIds = firstFileMessages.map(
            (firstFileMessage) => firstFileMessage.$.id
          );
          const sourceFileIds = sourceMessages.map(
            (sourceFileMessage) => sourceFileMessage.$.id
          );

          // check for removals
          firstFileMessages.forEach((firsFileMessage, index) => {
            const notExistingInSourceFile = sourceFileIds.indexOf(
              firsFileMessage.$.id
            );
          ...
          sourceMessages.forEach((sourceMessage, index) => {
            const notExistingInFirstFileIndex = firstFileSourceIds.indexOf(
              sourceMessage.$.id
            );
              ...
              async.waterfall(
                [
                  ...
                  (translatedBodies, callback) => {
                    xlfFileProcessor.getXlfFileByName(
                      fileName,
                      (err, xlfFile) => {
                        if (translatedBodies && translatedBodies.length) {
                          xlfFile.xliff.file[0].body[0]["trans-unit"] =
                            xlfFile.xliff.file[0].body[0]["trans-unit"].concat(
                              translatedBodies
                            );
                        }
                  ...
                  (xlfFile, translatedBodies, callback) => {
                    this.removeAndUpdateMessagesFromCsvAndTargetFile(
                      translatedBodies,
                      removeMessages,
                      xlfFile,
                      fileName,
 ...
 * @param callback
 */
XlfProcessor.prototype.removeAndUpdateMessagesFromCsvAndTargetFile = function (
  newMessages,
  removeMessages,
  xlfFile,
  file,
  ...
  async.waterfall(
    [
      (callback) => {
        ...
        xlfFileProcessor.readAndParseCsvToJson(
          csvFilePath,
          (err, csvMessages) => {
            // first remove the messages
            const removeMessagesIds = removeMessages.map(
              (removeMessage) => removeMessage.$.id
            );
            const tempMessages = [];
            ...
            const newCsvMessages = newMessages
 ...
 * @param callback
 */
XlfProcessor.prototype.translateFilesFromManualCsvTranslations = function (
  callback
) {
  let totalMutated = 0;
  async.waterfall(
    [
      (callback) => {
        xlfFileProcessor.listFiles(
          `${appRoot}${translatorConfig.outputPath}/translations/${constants.CSV}/`,
          (err, files) => {
            callback(err, files);
          }
        );
    ...
    (err) => {
      if (totalMutated > 0) {
        console.log(
          logSymbols.success,
          chalk.gray(`${totalMutated} files updated`)
        );
      } else {
        console.log(
          logSymbols.success,
          chalk.gray("No updates made in csv files")
        );
      }
      callback(err);
    }
 ...
 * @param callback
 */
XlfProcessor.prototype.validateAndChangeTargetsIfNeeded = function (
  file,
  callback
) {
  let totalMutated = 0;
  ...
  async.waterfall(
    [
        ...
        xlfFileProcessor.readXlfFile(xlfFilePath, (err, xlfFile) => {
          if (!xlfFile) {
            const error = new Error(
              `${errors.COULD_NOT_READ.description}, for path: ${xlfFilePath}`
            );
            return callback(error);
          }

          messageFile = xlfFile;
          const messageTranslationArray =
            xlfFile.xliff.file[0].body[0]["trans-unit"];
      ...
      (fileName, csvTranslationArray, messageTranslationArray, callback) => {
        ...
        const messageTargets = messageTranslationArray.map(
          (messageTranslation) => {
        ...
        csvTranslationArray.forEach((translation, index) => {
          ...
          if (!messageTargets.includes(target)) {
            if (messageTranslationArray[index]) {
              ...
              console.log(
                chalk.yellow(
                  `+++ ${target} for id ${messageTranslationArray[index].$.id}`
                )
              );
            } else {
              console.warn(
                logSymbols.warning,
                chalk.yellow("A target was not provided")
              );
            }
          }
        });
      ...
      (fileName, translationsArray, callback) => {
        ...
        xlfFileProcessor.createXlfFile(
          `${appRoot}${translatorConfig.outputPath}/${constants.OUTPUT_FILE_NAME}/${fileName}.${constants.FILE_TYPE}`,
          xml,
          (err) => {
